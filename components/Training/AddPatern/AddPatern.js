      ...
      <motion.div initial={{ x: 200 }} whileInView={{ x: 0 }} transition={{ duration: .2 }} className={css.modalWind} onClick={e => e.stopPropagation()}>
        ...
        <div className={css.headerContainer}>
          <div className={css.navBarContainer}>
            <h2 className={typePage === 'Упражнения' ? `${css.header1} ${css.header1Active}` : css.header1} onClick={() => setTypePage('Упражнения')}>Упражнения</h2>
            <h2 className={typePage === 'Тесты' ? `${css.header1} ${css.header1Active}` : css.header1} onClick={() => setTypePage('Тесты')}>Тесты</h2>
          </div>
                  ...
                  <div className={css.row}>
                    ...
                    <div className={css.textContainer}>
                      <h2 className={css.exerciseName}>{TextWrang1(exercise.nameRu)}</h2>
                      {
                        ...
                        <span className={css.cardPreHeader}>{globalExersicesArray.find(el => el.exersiceId == exercise.id)?.sets?.length}x{globalExersicesArray.find(el => el.exersiceId == exercise.id)?.sets[0]?.diapazonOt
                        }/{globalExersicesArray.find(el => el.exersiceId == exercise.id)?.sets[0]?.diapazonDo}</span>
                      }
                    </div>
                  </div>
                  {!exesiceId.includes(exercise.id) ?
                    <>
                      ...
                      <div className={css.buttonContainer}>
                        ...
                        {
                          globalExersicesArray.find(el => el.exersiceId == exercise.id) ?
                            <span className={css.addBtn} onClick={() => addExercise(exercise.id)}>Добавить</span>
                            :
                    ...
                    <div className={css.addOk} onClick={() => deleteExercises(exercise.id)}> Добавлено</div>
                ...
                <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} key={test.id} className={exesiceId.includes(test.id) ? `${css.exerciseCard} ${css.exerciseCardAdded}` : `${css.exerciseCard}`}>
                  ...
                  <div className={css.row1}>
                    <div className={css.textContainer}>
                      ...
                      <span className={css.cardPreHeader}>Норматив: {test.type}  {test.item}</span>
                    ...
                    {!testsId.includes(test.id) ?
                      <>
                        <div className={css.buttonContainer1}>
                          {/* <span className={css.updateBtn} onClick={() => openUpdateModal(exercise)}>Изменить</span> */}
                          <span className={css.addBtn} onClick={() => addTest(test.id)}>Добавить</span>
                        </div>
                      </> :
                      <div className={css.addOk1} onClick={() => deleteTest(test.id)}> Добавлено</div>
...
function SetsModalCard({ updateCard, setModalUpdateExercise, globalExersicesArray, setGlobalExersicesArray }) {
  ...
  useEffect(() => {
    let count = 0
    globalExersicesArray.find(el => el.exersiceId == updateCard.id)?.sets?.forEach(set => {
    ...
    setTime(globalExersicesArray.find(el => el.exersiceId == updateCard.id)?.time)
  ...
  const addExersicePokazaleli = () => {
    ...
    if (globalExersicesArray.find(el => el.exersiceId == updateCard.id)) {
      setGlobalExersicesArray([...globalExersicesArray.filter(el => el.exersiceId != updateCard.id), {
