...
const UpdateExercise1 = () => {
  ...
  useEffect(() => {
    ...
    if (sets) {
      ...
      setRecomendation(mobx.cardUpdateExercise?.body.recomendation)
    }
  ...
  const addExersicePokazaleli = () => {
    ...
    mobx.setFinalExersiceArrayOnDragAndDrop([
      ...mobx.finalExersiceArrayOnDragAndDrop.filter((el) => el.id != updateCard.id),
      {
        ...updateCard,
        body: {
        ...
        <div className={css.modalUpdateWind}>
          <div className={css.exitRow}>
            <div
              ...
              onClick={() => {
                mobx.setCardUpdateExerciseFlag(false);
                mobx.setCardUpdateExercise({});
              }}
            >{`< Назад`}</div>
            <div className={css.btn} onClick={deleteExercise}>
              <Trash2 className={css.img} />
            </div>
          </div>
          ...
          <div className={css.modalHeaderNavCotainer}>
            ...
            <span className={css.flex2}>
              {TextWrang(exercise?.pocazatel1Name)},{exercise?.pocazatel1Type}/диапазон
            </span>
          ...
          {array.slice(0, setCount).map((card) => (
            <SetsRow setNum={card[0]} updateCard={exercise} diapazonOt={card[1]} setDiapazonOt={card[2]} diapazonDo={card[3]} setDiapazonDo={card[4]} pokazatel2={card[5]} setPokazatel2={card[6]} pokazatel3={card[7]} setPokazatel3={card[8]} pokazatel4={card[9]} setPokazatel4={card[10]} pokazatel5={card[11]} setPokazatel5={card[12]} />
          ))}
...
export default UpdateExercise1;
