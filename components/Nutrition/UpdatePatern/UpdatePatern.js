...
const UpdatePatern = () => {
  ...
  useEffect(()=>{
    mobx.OnePattern.name && setName(mobx.OnePattern.name)
    mobx.OnePattern.description && setDescription(mobx.OnePattern.description)
    mobx.OnePattern.recomendation && setRecomendation(mobx.OnePattern.recomendation)
    if(mobx.OnePattern.name1){
      setName1(mobx.OnePattern.name1)
      setName1Flag(true)
    }
    mobx.OnePattern.description1 && setDescription1(mobx.OnePattern.description1)
    if(mobx.OnePattern.name2){
      setName2(mobx.OnePattern.name2)
      setName2Flag(true)
    }
    mobx.OnePattern.description2 && setDescription2(mobx.OnePattern.description2)
    if(mobx.OnePattern.name3){
      setName3(mobx.OnePattern.name3)
      setName3Flag(true)
    }
    mobx.OnePattern.description3 && setDescription3(mobx.OnePattern.description3)
    if(mobx.OnePattern.name4){
      setName4(mobx.OnePattern.name4)
      setName4Flag(true)
    }
    mobx.OnePattern.description4 && setDescription4(mobx.OnePattern.description4)
    if(mobx.OnePattern.name5){
      setName5(mobx.OnePattern.name5)
      setName5Flag(true)
    }
    mobx.OnePattern.description5 && setDescription5(mobx.OnePattern.description5)
    if(mobx.OnePattern.name6){
      setName6(mobx.OnePattern.name6)
      setName6Flag(true)
    }
    mobx.OnePattern.description6 && setDescription6(mobx.OnePattern.description6)
    if(mobx.OnePattern.name7){
      setName7(mobx.OnePattern.name7)
      setName7Flag(true)
    }
    mobx.OnePattern.description7 && setDescription7(mobx.OnePattern.description7)
  },[])
  ...
  const save = async() => {
    ...
    mobx.OnePattern.id ? formData.append('id', mobx.OnePattern.id) : formData.append('id', 111)
    ...
    const data = await PatternUtills.update(formData)
    if(data === 'ok'){
  ...
  return (
    <motion.div initial={{onPaste:0}} whileInView={{opacity:1}} className={css.container} onClick={()=>mobx.setUpdatePattern(false)}>
        <motion.div initial={{x:200}} whileInView={{x:0}} transition={{duration:.2}} className={css.modalWind} onClick={e => e.stopPropagation()}>
            <div className={css.headerContainer}>
                 <h2 className={css.header}>Редактировать шаблон</h2>
                 <div className={css.btn}>
                    <Image src={deleteImg} alt='' className={css.img} onClick={()=>PatternUtills.delete(mobx.OnePattern.id)}/>
                </div>
            </div>
            <div>
              ...
              <div className={css.headerContainer}>
                 ...
                 <div className={css.btn}>
                    <Image src={plus} alt='' className={css.img} onClick={add}/>
                </div>
            </div>
            {name1Flag&&
