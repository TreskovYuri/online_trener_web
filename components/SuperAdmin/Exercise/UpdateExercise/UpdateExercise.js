...
const UpdateExercise = () => {
  ...
  useEffect(()=>{
    ...
    mobx.oneExercise.descriptionRu && setDescriptionRu(mobx.oneExercise.descriptionRu)
    mobx.oneExercise.descriptionEng && setDescriptionEng(mobx.oneExercise.descriptionEng)
    mobx.oneExercise.link && setLink(mobx.oneExercise.link)
    mobx.oneExercise.equipment && setFinalEquipments(JSON.parse(mobx.oneExercise.equipment))
    mobx.oneExercise.stage && setStage(JSON.parse(mobx.oneExercise.stage))
    mobx.oneExercise.musclegroups && setFinalMuscleGrous(JSON.parse(mobx.oneExercise.musclegroups))
    mobx.oneExercise.img && setImageURL(`${process.env.NEXT_PUBLIC_STATIC_URL}/assets/${mobx.oneExercise.img}`)
    mobx.oneExercise.video && setVideoURL(`${process.env.NEXT_PUBLIC_STATIC_URL}/assets/${mobx.oneExercise.video}`)

    mobx.oneExercise.pocazatel1Name && setPocazatel1Name(mobx.oneExercise.pocazatel1Name)
    mobx.oneExercise.pocazatel1Type && setPocazatel1Type(mobx.oneExercise.pocazatel1Type)
    mobx.oneExercise.pocazatel1Name && setPocazatel1SPFlag(mobx.oneExercise.pocazatel1Name)
    mobx.oneExercise.pocazatel2Name && setPocazatel2Name(mobx.oneExercise.pocazatel2Name)
    mobx.oneExercise.pocazatel2Name && setPocazatel2Flag(true)
    mobx.oneExercise.pocazatel2Type && setPocazatel2Type(mobx.oneExercise.pocazatel2Type)
    mobx.oneExercise.pocazatel2Name && setPocazatel2SPFlag(mobx.oneExercise.pocazatel2Name)
    mobx.oneExercise.pocazatel3Name && setPocazatel3Name(mobx.oneExercise.pocazatel3Name)
    mobx.oneExercise.pocazatel3Name && setPocazatel3Flag(true)
    mobx.oneExercise.pocazatel3Type && setPocazatel3Type(mobx.oneExercise.pocazatel3Type)
    mobx.oneExercise.pocazatel3Name && setPocazatel3SPFlag(mobx.oneExercise.pocazatel3Name)
    mobx.oneExercise.pocazatel4Name && setPocazatel4Name(mobx.oneExercise.pocazatel4Name)
    mobx.oneExercise.pocazatel4Name && setPocazatel4Flag(true)
    mobx.oneExercise.pocazatel4Type && setPocazatel4Type(mobx.oneExercise.pocazatel4Type)
    mobx.oneExercise.pocazatel4Name && setPocazatel4SPFlag(mobx.oneExercise.pocazatel4Name)
    mobx.oneExercise.pocazatel5Name && setPocazatel5Name(mobx.oneExercise.pocazatel5Name)
    mobx.oneExercise.pocazatel5Name && setPocazatel5Flag(true)
    mobx.oneExercise.pocazatel5Type && setPocazatel5Type(mobx.oneExercise.pocazatel5Type)
    mobx.oneExercise.pocazatel5Name && setPocazatel5SPFlag(mobx.oneExercise.pocazatel5Name)
  },[])
  ...
  const save = async () => {
    ...
    mobx.oneExercise.id && formData.append('id', mobx.oneExercise.id)
    ...
    const data = await TrainingUtills.updateExercise(formData)
    if (data === 'ok') {
  ...
  return (
    <motion.div initial={{ onPaste: 0 }} whileInView={{ opacity: 1 }} className={css.container} onClick={() => mobx.setUpdateExercise(false)}>
      <motion.div initial={{ x: 200 }} whileInView={{ x: 0 }} transition={{ duration: .2 }} className={css.modalWind} onClick={e => e.stopPropagation()}>
        ...
        <div className={css.headerContainer}>
          ...
          {page == 4 ?
            ...
            <h2 className={css.header}>Изменение упражнения</h2>
          }
          <div className={css.btnContainer}>

            {
              ...
              :
              <div className={css.btn} onClick={()=>TrainingUtills.deleteExercise(mobx.oneExercise.id)}>
  ...
  return (
    <div>
      ...
      <div className={css.equipmentContainner}>
        <span className={css.equipment} onClick={() => addDelEquipments('Разминка')}>Разминка{stage.includes('Разминка') && <Image src={arrow}  className={css.arrow}/>}</span>
        <span className={css.equipment} onClick={() => addDelEquipments('Упражнение')}>Упражнение{stage.includes('Упражнение') && <Image src={arrow}  className={css.arrow}/>}</span>
        <span className={css.equipment} onClick={() => addDelEquipments('Заминка')}>Заминка{stage.includes('Заминка') && <Image src={arrow}  className={css.arrow}/>}</span>
      </div>
    ...
    <div>
      ...
      <div className={css.fileInput}>
        ...
        <label htmlFor="file1" className={css.labelFile}>
          {videofile &&
            <video alt={`Аватар пользователя: `} className={css.trainingVideo} width={10} height={10} unoptimized><source src={videoURL ? videoURL : `${process.env.NEXT_PUBLIC_STATIC_URL}/assets/video.aca`} type="video/mp4" /></video>
          }
          {!videofile &&
            videoURL?
            <video alt={`Аватар пользователя:`} className={css.trainingVideo} width={10} height={10} unoptimized><source src={videoURL} type="video/mp4" /></video>
            :
